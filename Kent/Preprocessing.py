...
class Kent_Preprocessing(BaseEstimator):
    ...
    def finalProcessing(self, body):
        ...
        def assign_general_tag(scored_probabilities):
            if scored_probabilities < 0.18:
                return 'PT_ENP_TagAXXXX_LOWXXXXX'
            elif scored_probabilities < 0.36:
        ...
        # This will give you a dataframe similar to what you would get from the SQL query
        result_df = df.copy()
        ...
        result_df['Assigned To'] = np.where(result_df['Scored Probabilities'] < 0.18, 'LOW', 
                                            np.where(result_df['Scored Probabilities'] > 0.36, 'HIGH', 'MED'))
        ...
        result_df['Scored Labels'] = np.where(result_df['Scored Probabilities'] < 0.18, 0, 1)
