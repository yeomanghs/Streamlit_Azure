...
import pandas as pd
import re
from Birmingham.Config import *
import urllib.request
import json
from sklearn import preprocessing
from sklearn.cluster import KMeans

class Birmingham_Preprocessing(BaseEstimator):
    ...
    def transform(self, df = None):
        ...
        df.rename(columns = {"tag_value":"Tagvalue"}, inplace = True)

        #harmonize tag_value
        def getTagValueGroup(tagValueArray):
            resultList = []
            for value in tagValueArray:
                if re.search('^IDP', str(value)):
                    group = 'IDP-related'
                elif re.search('^MO UOB.*CO', str(value)):
                    group = 'MO UOB-related and CO'
                elif re.search('^MO UOB.*UO', str(value)):  
                    group = 'MO UOB-related and UO'
                else:
                    group = 'Others'
                resultList.append(group)
            return resultList

        df['TagValueGroup'] = getTagValueGroup(df['Tagvalue'].values)
    ...
    def finalProcessing(self, body):
        ...
        taskType = 'Made Offer'
        df = self.df.copy()
        for col in ['Tagvalue', 'IntakeStatus', 'InitialOfferStatus', 'Year', 'COR', 'Campus']:
            dfResult[col] = df[col]
